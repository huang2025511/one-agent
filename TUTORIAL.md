...
> 本教程将带你从零开始安装、配置并运行 One-Agent，覆盖所有主要功能的使用方法。
...
10. [钉钉接入](#10-钉钉接入)
11. [飞书接入](#11-飞书接入)
12. [Discord 接入](#12-discord-接入)
13. [Slack 接入](#13-slack-接入)
14. [四层智能路由详解](#14-四层智能路由详解)
15. [三层记忆系统](#15-三层记忆系统)
16. [技能系统](#16-技能系统)
17. [执行环境（Shell / Docker / 浏览器）](#17-执行环境shell--docker--浏览器)
18. [定时任务调度](#18-定时任务调度)
19. [监控仪表盘](#19-监控仪表盘)
20. [多模态功能（图像/语音）](#20-多模态功能图像语音)
21. [插件市场](#21-插件市场)
22. [数据加密](#22-数据加密)
23. [冒烟测试](#23-冒烟测试)
24. [常见问题](#24-常见问题)
...
在群聊中添加自定义机器人，获取 Webhook URL 后即可向群聊推送消息。
...
**获取 Webhook URL：**
1. 打开钉钉群聊 → 右上角 `⋯` → 智能群助手 → 添加机器人 → 自定义
2. 安全设置选择"加签"，复制 Webhook URL 和签名密钥

**使用：**

```python
# 发送文本消息
result = await app.dingtalk.send_webhook("任务完成！", at_all=False)

# 发送 Markdown 消息
result = await app.dingtalk.send_markdown("日报", "## 日报\n> 完成项: 5\n> 待处理: 2")
```

### 10.2 Stream 模式（企业内部应用）

通过钉钉企业内部应用，实现**接收用户消息并自动回复**（流式响应）。
...
1. 登录 [钉钉开发者后台](https://open-dev.dingtalk.com/) → 创建应用
2. 获取 AppKey（client_id）和 AppSecret（client_secret）
3. 启用机器人能力，配置消息接收模式为 Stream 模式
...
gateways:
  feishu:
    enabled: true
    mode: "webhook"
    webhook_url: "https://open.feishu.cn/open-apis/bot/v2/hook/你的HookID"
    secret: "你的签名验证密钥"
```
...
```bash
export FEISHU_WEBHOOK_URL="https://open.feishu.cn/open-apis/bot/v2/hook/你的HookID"
export FEISHU_SECRET="你的签名验证密钥"
```

**获取 Webhook URL：**
1. 打开飞书群聊 → 设置 → 群机器人 → 添加机器人 → 自定义机器人
2. 复制 Webhook URL，安全设置选择"签名校验"

**使用：**

```python
# 发送文本消息
result = await app.feishu.send_webhook("任务完成！")

# 发送富文本消息
result = await app.feishu.send_rich_text("日报", [["完成项: 5", "待处理: 2"]])
```

### 11.2 App 模式（自建应用）

通过飞书自建应用，实现**接收用户消息并自动回复**。
...
1. 登录 [飞书开发者后台](https://open.feishu.cn/app) → 创建应用
2. 添加机器人能力
3. 配置事件订阅，设置回调 URL：`http://你的服务器:18795/feishu/callback`
4. 获取 App ID、App Secret、Verification Token、Encrypt Key
...
3. 左侧菜单选择 **Bot** → 点击 **Reset Token** → 复制 Token
...
gateways:
  discord:
    ...
    allowed_channels: ["1234567890"]   # 允许的频道 ID 列表，留空允许所有
```
...
2. 点击 **Create New App** → **From scratch** → 输入名称并选择 Workspace
3. 在 **Bot Token Scopes** 中添加：`chat:write`、`channels:history`、`channels:read`
4. 点击 **Install to Workspace** → 复制 Bot Token（格式：`xoxb-...`）
...
gateways:
  slack:
    enabled: true
    bot_token: "xoxb-你的BotToken"
    app_token: "xapp-你的AppToken"      # 可选，启用 Socket Mode 时需要
    allowed_channels: ["C1234567890"]   # 允许的频道 ID 列表，留空允许所有
```
...
启动 Agent 后，Bot 会自动连接。在允许的频道中 @Bot 即可对话。
