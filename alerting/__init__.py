...
class AlertManager(Plugin):
    ...
    async def setup(self, ctx) -> None:
        ...
        for ch_cfg in channels_cfg:
            ...
            if ch_type == "webhook":
                ...
                self._channels.append(self._make_email_channel(ch_cfg))
            elif ch_type == "wechat_work":
                self._channels.append(self._make_wechat_work_channel(ch_cfg))
            elif ch_type == "dingtalk":
                self._channels.append(self._make_dingtalk_channel(ch_cfg))
            elif ch_type == "feishu":
                self._channels.append(self._make_feishu_channel(ch_cfg))
    ...
    def _make_email_channel(self, cfg: Dict[str, Any]) -> Callable:
        """创建 SMTP 邮件通知渠道。"""
        # 邮件服务器配置
        ...
        def _send_sync(alert: AlertEvent) -> None:
            # 同步发送邮件（在线程池中执行）
            from email.mime.text import MIMEText
            import smtplib

            subject = f"[One-Agent 告警] {alert.severity} - {alert.rule_name}"
            body = (
    ...
    def _make_wechat_work_channel(self, cfg: Dict[str, Any]) -> Callable:
        """创建企业微信群机器人通知渠道。"""
        webhook_url = cfg.get("webhook_url", "")
        ...
        async def send(alert: AlertEvent):
            ...
            try:
                ...
                logger.warning("wechat_work alert failed: %s", exc)

        return send

    def _make_dingtalk_channel(self, cfg: Dict[str, Any]) -> Callable:
        """创建钉钉群机器人通知渠道（支持签名认证）。"""
        webhook_url = cfg.get("webhook_url", "")
        ...
        async def send(alert: AlertEvent):
            ...
            url = webhook_url
            # 如果配置了 secret，计算 HMAC-SHA256 签名并拼接到 URL
            if secret:
                import hmac
                import hashlib
                import base64
    ...
    def _make_feishu_channel(self, cfg: Dict[str, Any]) -> Callable:
        """创建飞书群机器人通知渠道。"""
        webhook_url = cfg.get("webhook_url", "")
        ...
        async def send(alert: AlertEvent):
            ...
            try:
                ...
                logger.warning("feishu alert failed: %s", exc)
