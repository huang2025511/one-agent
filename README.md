...
主配置文件：`config/default_config.yaml`（YAML 格式）
...
llm:
  ...
  api_keys:
    openai: null
    deepseek: null
    sensenova: sk-xxxxx    # 支持 ${ENV_VAR} 环境变量引用
...
REST API 运行在 `http://127.0.0.1:18792`，Swagger 文档在 `/docs`。
...
| GET | `/api/settings?key=…` | 读取配置 |
| POST | `/api/settings` | 修改配置 |
